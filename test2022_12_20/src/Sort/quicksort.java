...
public class quicksort implements Sort{
    ...
    public static void quicksort(int[] array,int left,int right){
        ...
        //优化2 三数取中
        int mid=threeMidIndex(array,left,right);
        int tmp=array[left];
        array[left]=array[mid];
        array[mid]=tmp;

        //确定中间节点
        int privo=maxIndex(array,left,right);

        quicksort(array,left,privo-1);
        quicksort(array,privo+1,right);
    }

    public static int threeMidIndex(int[] array,int left,int right){
    ...
    public static int maxIndex(int[] array,int left,int right){
        int tmp=array[left];
        while(left<right){
